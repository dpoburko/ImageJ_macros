...
_v5d - came back to using background subtraction before ftt alignment. This appears to be critical for high accuracy
...
cummulatedxf = newArray(dxf.length);
cummulatedyf = newArray(dyf.length);
framesList =newArray(dyf.length);
for(d=0;d<dxf.length;d++) {
	sumdxf = 0;
	sumdyf = 0;
	for(s=0;s<d;s++) {
	 	sumdxf = sumdxf+dxf[s];
	...
	cummulatedxf[d] = sumdxf;
	cummulatedyf[d] = sumdyf;
	framesList[d] = d; 
}
	...
	for (f=1;f<=frames;f++) {
		for (s=1;s<=slices;s++) {
			for (c=1;c<=channels;c++) {	
				...
				//translate(dsFactor*cummulatedxf[f-1] + dsFactor*dxc[(f-1)*channels+c-1] , dsFactor*cummulatedyf[f-1] + dsFactor*dyc[(f-1)*channels+c-1]);
...
Plot.create("Deltas", "Frame", "Pixels", framesList, dxf);
Plot.setColor("blue");
Plot.add("line", framesList, dyf, "dyf");
Plot.setColor("black");
Plot.add("line", framesList, dxf, "dxf");
...
tEnd = getTime();
print("\\Update5: Image alignment complete in "+ d2s((tEnd-tStart)/1000,2)+" s.");
